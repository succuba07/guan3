...
class DenoisingDiffusion(object):
    ...
    def load_ddm_ckpt(self, load_path, ema=False):#加载权重文件pth
    ...
    # 修复键名：如果权重是为单卡保存的（无 "module." 前缀），则添加前缀以适配 DDP 模型
        new_state_dict = {}
        for k, v in state_dict.items():
            if not k.startswith('module.'):  # 判断是否需要添加前缀
                k = 'module.' + k  # 添加前缀
            new_state_dict[k] = v
    ...
    def train(self, DATASET):
        ...
        for epoch in tqdm(range(self.start_epoch, self.config.training.n_epochs)):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                lr_sched.adjust_learning_rate(self.optimizer, i / len(train_loader) + epoch, self.config)
            ...
            if ((epoch + 1) % self.config.training.snapshot_freq == 0) and dist.get_rank() == 0:
                ...
                state_dict = self.model.module.state_dict()#3.27修改添加module
